					...
					<div class="block"><xsl:if test="@used!=''"><xsl:attribute name="class">block opacity</xsl:attribute></xsl:if>
						<div class="right"> <xsl:if test="../ident=2"><xsl:attribute name="class">right long</xsl:attribute></xsl:if>
							...
							<span class="paid"> <form id="prtnForm2" method="post"> 
								<xsl:choose>
									<xsl:when test="@used!=''">
										<input name="couponId" type="hidden" value="{@uOfferId}"/> <input name="used" type="hidden" value="0"/> <input name="couponSbmt2" type="submit" class="bttn2" value="Отменить использование"/>
									</xsl:when>
									<xsl:when test="../ident=2"></xsl:when>
									<xsl:otherwise>
										<input name="couponId" type="hidden" value="{@uOfferId}"/> <input name="used" type="hidden" value="1"/> <input name="couponSbmt2" type="submit" class="bttn2" value="Отметить использованным"/>
									</xsl:otherwise>
								</xsl:choose>
							</form> </span>
						</div>

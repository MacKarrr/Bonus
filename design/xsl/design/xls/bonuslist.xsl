	...
	<script type="text/javascript">
		$(document).ready(function(){
			$("div.bonuscont").each(function(){
				...
				$(this).find('.time').countdown(new Date(2012, 03, <xsl:value-of select="item/sysdate/day"/>, 23, 59, 59), {prefix:'', finish: ' '});
			})
		});
	</script>
  ...
  <xsl:template name="list">
	...
	<div class="bonuscont"> <div class="bonusbg"></div>
		...
		<div class="min"><xsl:if test="discount>0"><xsl:attribute name="class">lineheight</xsl:attribute></xsl:if>
			Цена <xsl:if test="count(offer)>1">от </xsl:if> <strong> <xsl:value-of disable-output-escaping="yes" select="min"/>'<small><xsl:value-of disable-output-escaping="yes" select="min/@copeck"/></small> </strong> руб.
			<xsl:choose>
				<xsl:when test="percent!=''">
					| <span><xsl:if test="percent/@small=1"><xsl:attribute name="class">small</xsl:attribute></xsl:if><xsl:value-of disable-output-escaping="yes" select="percent"/></span>
				</xsl:when>
				<xsl:otherwise>
